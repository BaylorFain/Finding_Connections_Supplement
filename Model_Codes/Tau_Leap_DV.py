...
for i in range(n_simulations):
    ...
    with open('Data/Tau_Leaping_Model/Deterministic/TauLeap_DV_'+str(i)+'.txt', 'w+') as outfile:
    ...
    while timetime < endtime:
        ...
        V[i][0] = np.append(V[i][0], V[i][-1][-1] + leap*(Rate[3] - Rate[4]))
        ...
        if int((timetime)/tau)%save == 0:
            with open('Data/Tau_Leaping_Model/Deterministic/TauLeap_DV_'+str(i)+'.txt', 'a+') as outfile:
...
with open('Data/Tau_Leaping_Model/Deterministic/TauLeap_DV_average.txt', 'w') as outfile:
